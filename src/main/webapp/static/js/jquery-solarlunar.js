...
var open_type = -1; //表示日历被选类型，1：阳历，2：农历
(function ($) {
    $.fn.solarlunar = function (opts) {
        ...
        $("input[name=calendar"+solarlunar_num+"]").on("click", function () {
            if ($(this).val() == 1) { 
                ...
                var ri = $(this).parent().find(".day option:selected").text();set_open_type_value(1);
                obj.val(nian + yue + ri);
            } else { 
                ...
                var ri = $(this).parent().find(".lday option:selected").text();set_open_type_value(2);
                obj.val(nian + yue + ri);
            }
        });
    ...
    //设置open_type的同时为对应类型结点进行选择
    function set_open_type_value(set_value){
        open_type = set_value;
        switch(open_type){
            case 1 : 
                $("#add_thing_timeType_input2").prop('checked', 'false');
